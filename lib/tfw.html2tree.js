/*
 ...
 Convert an HTML string into an object tree représentation simply called `tree`.
...
module.exports = function(libs) {
    ...
    ['Fatal', 'Tree'].forEach(function (libname) {
        if (typeof libs[libname] === 'undefined') {
            throw Error('[tfw.html2tree] Missing mandatory library: "' + libname + '"!');
        }
    });
    ...
    return function (content) {
        ...
        function append(node) {
            if (node.type == libs.Tree.TEXT
                && root.children.length > 0
                && root.children[root.children.length - 1].type == libs.Tree.TEXT)
            {
        ...
        function flushText() {
            ...
            if (text.length > 0) {
                append({type: libs.Tree.TEXT, text: text});
            }
            cursor = index;
        }
        /**
        ...
        try {
            while (index < content.length) {
                if (!match(
                    [rxStartTag, function (m) {
                        flushText();
                        node = {type: libs.Tree.TAG, name: m[1], attribs: {}, children: [], pos: index};
                        index += m[0].length;
                        parseAttribs(node);
                        return match(
                    ...
                    [rxEntity, function (m) {
                        ...
                        flushText();
                        append({type: libs.Tree.ENTITY, text: m[0], pos: index});
                        cursor = index = index + m[0].length;
                        return true;
                    }],
                    [rxComment, function (m) {
                        flushText();
                        append({type: libs.Tree.COMMENT, text: m[1], pos: index});
                        index += m[0].length;
                        cursor = index;
                        return true;
                    }],
                    [rxDocType, function (m) {
                        flushText();
                        node = {type: libs.Tree.DOCTYPE, name: m[1], attribs: {}, pos: index};
                        index += m[0].length;
                        parseAttribs(node);
                    ...
                    [rxStartProcessing, function (m) {
                        flushText();
                        node = {type: libs.Tree.PROCESSING, name: m[1], attribs: {}, pos: index};
                        index += m[0].length;
                        parseAttribs(node);
            ...
            if (typeof ex.pos !== 'undefined') {
                libs.Fatal.fire(ex.msg + "\n\n" + libs.Fatal.extractCodeAtPos(content, ex.pos));
            } else {
                libs.Fatal.bubble(ex);
            }
        }
        return stack[0];
    };
...
// Regular expressions used to parse the HTML content.
var rxEntity = /^&[a-zA-Z]+;/;
